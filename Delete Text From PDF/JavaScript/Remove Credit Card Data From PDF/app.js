...
const API_KEY = "ADD_YOUR_PDFco_API_KEY_HERE";

// Source PDF file
const SourceFile = "./sample_credit_cards.pdf";
...
function deleteTextFromPdf(apiKey, uploadedFileUrl, password, destinationFile) {
    ...
    const regex_creditCard = '[0-9]{4} [0-9]{4} [0-9]{4} [0-9]{4}';

    // Regular expression CVV number
    const regex_cvv = '\\b[0-9]{3}\\b';

    // Regular expression mm/year
    const regex_month_year = '[0-9]{2}(\/|-)[0-9]{4}';
    ...
    var jsonPayload = JSON.stringify({
        ...
        url: uploadedFileUrl, 
        async: true, 
        searchStrings: [regex_creditCard, regex_cvv, regex_month_year],
        regex: true
    });
    ...
    var postRequest = https.request(reqOptions, (response) => {
        response.on("data", (d) => {
            ...
            if (data.error == false) {
                ...
                // Service reported error
                console.log("removeText(): " + data.message);
            }
        });
    })
        .on("error", (e) => {
            // Request error
            console.log("removeText(): " + e);
        });
