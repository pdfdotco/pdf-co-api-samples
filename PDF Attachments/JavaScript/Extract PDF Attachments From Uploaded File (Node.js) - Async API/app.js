...
getPresignedUrl(API_KEY, SourceFile)
    .then(([uploadUrl, uploadedFileUrl]) => {
        ...
        uploadFile(API_KEY, SourceFile, uploadUrl)
            .then(() => {
                // 3. EXTRACT ATTACHMENTS FROM PDF
                extractPdfAttachments(API_KEY, uploadedFileUrl);
            })
            .catch(e => {
                console.log(e);
            });
...
function extractPdfAttachments(apiKey, uploadedFileUrl) {
    // Prepare request for API endpoint
    var queryPath = `/v1/pdf/attachments/extract`;

    // JSON payload for api request
    var jsonPayload = JSON.stringify({
        url: uploadedFileUrl, async: true
    });
    ...
    var postRequest = https.request(reqOptions, (response) => {
        response.on("data", (d) => {
            ...
            if (data.error == false) {
                ...
                // Service reported error
                console.log("extractPdfAttachments(): " + data.message);
            }
        });
    })
        .on("error", (e) => {
            // Request error
            console.log("extractPdfAttachments(): " + e);
        });
    // Write request data
